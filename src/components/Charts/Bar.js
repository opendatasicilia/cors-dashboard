    ...
    useEffect(() => {
        async function get() {
            ...
            if(mode === 'incidenza'){
                ...
                const vaccinati = res.map(a => new Date(a.data) > new Date('2021-12-08') ? (a['%vaccinati'] * target['>=5'] / 100) : a['%vaccinati'] * target['>=12'] / 100).map((v, i, a) => v - (a[i - 1] || 0))
                const immunizzati = res.map(a => new Date(a.data) > new Date('2021-12-08') ? (a['%immunizzati'] * target['>=5'] / 100) : a['%immunizzati'] * target['>=12'] / 100).map((v, i, a) => v - (a[i - 1] || 0))
                setData({
                  date: date.slice(1),
